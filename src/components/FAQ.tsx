                  ...
                  return (
                    <span key={lineIndex} className="block mb-2 last:mb-0">
                      {parts.map((part, partIndex) => {
                        const shouldBeBold = /\«Правильная тарелка\»|Видео МФР|Уход за кожей|щёток для тела|ссылки|обновление|Цена - это про ценность|полностью просчитанный КБЖУ для каждого блюда; гайд по уходу за кожей; МФР-упражнения|картинки с пояснениями|схему «правильной тарелки»|комьюнити с поддержкой и челленджами/.test(part);
                        return shouldBeBold ? (
