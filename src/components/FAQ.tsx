          ...
          {faqItems.map((item, index) => (
            ...
            >
              ...
              <p className={`text-xs lg:text-base leading-relaxed font-manrope ${item.textColor} opacity-90`}>
                {item.answer.split('\n').map((line, lineIndex) => {
                  // Обрабатываем жирный текст в скобках
                  const parts = line.split(/(\«[^»]+\»|щёток для тела|ссылки|обновление|Цена - это про ценность|полностью просчитанный КБЖУ|МФР-упражнения|картинки с пояснениями|схему «правильной тарелки»|комьюнити с поддержкой и челленджами)/g);
                  
                  return (
                    <span key={lineIndex} className="block mb-2 last:mb-0">
                      {parts.map((part, partIndex) => {
                        const shouldBeBold = /«[^»]+»|щёток для тела|ссылки|обновление|Цена - это про ценность|полностью просчитанный КБЖУ|МФР-упражнения|картинки с пояснениями|схему «правильной тарелки»|комьюнити с поддержкой и челленджами/.test(part);
                        return shouldBeBold ? (
